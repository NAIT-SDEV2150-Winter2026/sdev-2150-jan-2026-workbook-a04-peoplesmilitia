...
renderTaskList = function(items) {
  // Stretch Goal:  - Display a summary line above the list
  const completedCount = items.filter(t => t.done).length;
  let html = `<p><strong>Completed: ${completedCount} of ${items.length}</strong></p>`;
  
  html += '<ul>';
  for (const item of items) {
    const statusClass = item.done ? 'done' : 'todo';
    const statusText = item.done ? '(done)' : '(todo)';
    // Stretch Goal: - Display a chekcbox next to each task to represent done/todo (checking/unchecking it toggles the state)
    html += `
      <li class="${statusClass}" style="cursor:pointer">
...
list.addEventListener('click', (event) => {
  ...
  const title = li.textContent.replace(' (done)', '').replace(' (todo)', '').trim();
...
txtTask.addEventListener('keypress', (event) => {
  if (event.key === 'Enter') {
    btnAdd.click(); 
  }
});