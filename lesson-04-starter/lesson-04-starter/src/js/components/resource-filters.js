...
class ResourceFilters extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._formEl.addEventListener('input', this._handleSubmit); 
    this._formEl.addEventListener('change', this._handleSubmit);
    const resetBtn = this.shadowRoot.querySelector('.btn-outline-secondary');
...
_handleReset() {
    ...
    const buttons = this._categoryGroupEl.querySelectorAll('.btn');
  ...
  _handleSubmit(event){
    ...
    this.dispatchEvent(new CustomEvent('resource-filters-changed', {
      detail: filters,
      bubbles: true,
      composed: true,
    }));
  ...
  _handleCategorySelection(event){
    const butttonThatWasClickedOn = event.target.closest('.btn');
    if(butttonThatWasClickedOn){
      const activeSibling = butttonThatWasClickedOn.parentElement.querySelector('.active');
      activeSibling.classList.remove('active');
      butttonThatWasClickedOn.classList.add('active');
      this._handleSubmit()
    }
    //alert(butttonThatWasClickedOn);
  }
  render() {
